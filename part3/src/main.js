...
function hit_sphere(center, radius, ray) {
    ...
    let dot_oc_direction = glMatrix.vec3.dot(oc, ray.direction); // квадрат катета (луча)
    ...
    let b = Math.sqrt(dot_c_c - dot_oc_direction * dot_oc_direction);
    let t2hc = radius - b; // сравниваем растояние до луча с радиусом если меньше чем ноль згначить лучь прошел мимо сферы. 
    if (t2hc < 0) {
