...
function main() {
  ...
  let sphere2 = new Sphere(glMatrix.vec3.fromValues(1.0, 0.0, -1.0), 0.5, material_right);
  let sphere3 = new Sphere(glMatrix.vec3.fromValues(-1.0, 0.0, -1.0), 0.5, material_left);
  ...
  for (let j = 0; j < image_heigth; j += 1) {
    for (let i = 0; i < image_width; i += 1) {
      ...
      for (let index = 0; index < samples_per_pixel; index++) {
        // let u = ((i + Math.random() * 0.5) / image_width - 0.5) * 2.0;
